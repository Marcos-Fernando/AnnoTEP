...
def annoSINE(new_filename, folderSINE, seedSINE):
    ...
    print("Pasta criada para armazenamento dos dados: ", folderSINE)
    ...
    print("seedSINE criada:", seedSINE )
...
def annoLINE(new_filename, folderSINE, folderLINE, resultsLINE, libLINE):
    ...
    print("libLINE criada:", libLINE )
    print("Anotação LINE finalizado")

def annoMSC(new_filename, folderSINE, seedSINE, folderEDTA, libLINE):
    ...
    wait
    cat sort_occur.txt | cut -f 1,2 | sed 's#^#id=\\\"#g' | sed 's#\\t#\\\" ; data=\\\"#g' | sed 's#$#\\\" ; ver="`cat ids.txt | grep $id`" ; echo -e "$ver\\t$data" #g' > occpick.sh
    bash occpick.sh | grep "^TE" | grep "^TE" | sed 's/#/_/g' | sed 's#/#_#g' > occurrences.tsv

    cat sort_size.txt | cut -f 1,3 | sed 's#^#id=\\\"#g' | sed 's#\\t#\\\" ; data=\\\"#g' | sed 's#$#\\\" ; ver="`cat ids.txt | grep $id`" ; echo -e "$ver\\t$data" #g' > sizepick.sh
    bash sizepick.sh | grep "^TE" | grep "^TE" | sed 's/#/_/g' | sed 's#/#_#g' > size.tsv

    rm -f pick-occur.sh sort_occur.txt sort_size.txt ids.txt occpick.sh sizepick.sh
...
def upload_file():
    if request.method == 'POST':
     ...
     #       try:
     #           annoMSC(new_filename, folderSINE, seedSINE, folderEDTA, libLINE)
     #       except:
     ...
     #           return redirect(request.url)
        ...
        filenamedb = request.form.get('filename')
