#!/home/amvarani/miniconda3/envs/EDTA-new/bin/perl
##---------------------------------------------------------------------------##
##  File:
##      @(#) ProcessRepeats
##  Authors:
...
foreach my $file ( @ARGV ) {
  ...
  if ( $catfile =~ /.*\.gz$/ ) {
    open( INCAT, "gunzip -c $catfile |" ) || die "Can\'t open file $catfile\n";
  }
  else {
    open( INCAT, $catfile ) || die "Can\'t open file $catfile\n";
  }
  ...
  while ( <INCAT> ) {
    ...
    elsif ( /^(##\s*)?RepeatMasker|run|RepBase|RM/ ) {
      my @bit = split;
    ...
    }elsif ( /^##\s+RAW Annotations/ ) {
      last;
    }
  }
  close INCAT;

  print "ProcessRepeats: Adjudicating alignments\n";

  # Do it all
  %aggregateStats = &parseCATFile ( $catfile, $engine, \%seq1Lengths, \%aggregateStats, 0 );

  &generateTableOutput( \%options, \%seq1Lengths, \%aggregateStats, 
                         $file, $filename, $dbversion, $numSearchedSeqs, 
...
sub parseCATFile {
  ...
  my %aggregateStats = %{ shift() };
  my $noAlignData = shift;

  my $INCAT;
  if ( $file =~ /.*\.gz$/ ) {
    open( $INCAT, "gunzip -c $file |" )
  ...
  # Glocal variables ( global to callback but local to parseCATFile )
  my $searchResults = SearchResultCollection->new();
  my $prevID = undef;
  my $prevRMBatchNum = undef;
  ...
  sub callback {
    my $record = shift;
    # recast
    bless $record, "PRSearchResult";
    $records_processed++;
    my $seq_id = $record->getQueryName();
    
    ## THERE ARE TWO CASES HERE:
    #      1. Large single sequence that has been split into multiple batches ( each
    #         with it's own refinement section ).
    #      2. A single batch with multiple sequences and a single refinement section.
    #
    #      Only process if we:
    #          - switch IDs and the next ID doesn't have the same batch number
    if ( $prevID && $seq_id ne "refinement" && $seq_id ne $prevID ) {
      print "Processing seq $prevID\n" if ( $DEBUG );
      
      if (    $searchResults->size()
           && $searchResults->get( 0 )->getLineageId !~ /\[?[mc]_b.*/ )
      {
        die "\n    The input file:\n        $file\n"
      ...
      %aggregateStats = &processSequence( $batchNum, $file, $engine, $searchResults, \%seq1Lengths, \%aggregateStats );
      $batchNum++;

      # Beware...global variable
      %refinementHash = ();
      $searchResults = undef;
      $searchResults = SearchResultCollection->new();
 
    }

    $record->setClassName( "Unspecified" )
        if ( $record->getClassName() eq "" );
    $record->setOrientation( "+" )
        if ( $record->getOrientation() ne "C" );

    if ( $seq_id eq "refinement" && $record->getLineageId() =~ /\[/ ) {
      my $catID = $record->getLineageId();
      die "Missing cat file ID for refined element!"
          . Dumper( $record ) . "\n"
          if ( $catID eq "" );
      $catID =~ s/[\[\]]//g;
      push @{ $refinementHash{$catID}->{ $record->getHitName() } },
          ( $record );
    }else {
      # PRID is only set for normal annotations ( not refinements )
      $record->setPRID( $annotation_idx );

      # Save original information for more detailed reporting ( if requested )
      if ( $options{'source'} ) {
        $record->addDerivedFromAnnot( $record );
        $record->setAlignData( undef );
      }

      # Remove TE variant suffixes
      my $HitName = $record->getHitName();
      $HitName =~ s/_short_$//;
      # RMH 11/21/12: Added for Dfam
      $HitName =~ s/_offset$//;
      $record->setHitName( $HitName );

      $searchResults->add( $record );
      my $seq_len = $record->getQueryEnd() + $record->getQueryRemaining(); 

      # Update sequence length hash ( longest if different! )
      $seq1Lengths{ $seq_id } = $seq_len
          if ( !defined $seq1Lengths{ $seq_id }
               || $seq1Lengths{ $seq_id } < $seq_len );
  ...
  unless ( $records_processed > 0 ) {
    my $filenaam = $file;
#####TODO move this block back up to the caller ( return num of records processed )
...
sub processSequence  {
  ...
  # File is the full path to the catfile,
  # convert file prefix
  $file =~ s/\.(temp)?cat(.gz)?$//;
  ...
  #%refinementHash = ();
##TODO: move this to the parseCATFile function
...
####TODO: This needs to be passed in to this function
#  my $i              = -1;
...
CYCLE2:
  ...
  #
  if ( !$poundformat ) {
    %aggregateStats = &generateOutput( \%options, \%seq1Lengths, 
                                   \%aggregateStats, $file,
                                   $engine, $sortedAnnotationsList,
                                   $batchNum );
  }
  else {
...
"Near duplicate (created by fusing), or short < 10bp fragment: Removing Current element:\n";
    ...
    #
    # Generate output for this cat file!
    #
    %aggregateStats = &generateOutput( \%options, \%seq1Lengths, 
                                   \%aggregateStats, $file,
                                   $engine, $sortedAnnotationsList,
...
###########################################################################################################
			 ...
			 'LTRNCROI'  	 => { 'RE' => '^LTR\/Gypsy\/non-chromo-like|^LTR\/Gypsy\/non-chromo-outgroup-like' },
			 ...
			 'LTRCRO'  	 => { 'RE' => '^LTR\/Gypsy\/chromo$|^LTR\/Gypsy\/chromo-outgroup$' },
                         'LTRCROI'       => { 'RE' => '^LTR\/Gypsy\/chromo-like|^LTR\/Gypsy\/chromo-outgroup-like' },
			 ...
			 'LTRTEK'  	 => { 'RE' => '^LTR\/Gypsy\/Tekay$|^LTR\/Gypsy\/Del$' },
                         'LTRTEKI'       => { 'RE' => '^LTR\/Gypsy\/Tekay-like|^LTR\/Gypsy\/Del-like' },                         

			 'LTRREI'  	 => { 'RE' => '^LTR\/Gypsy\/Reina$' },
                         'LTRREII'       => { 'RE' => '^LTR\/Gypsy\/Reina-like' },
                         
                         'LTRFERN'  	 => { 'RE' => '^LTR/Gypsy\/Ferney$' },			# new
                         'LTRFERNI'      => { 'RE' => '^LTR\/Gypsy\/Ferney-like' },		# new	
...
###############################################################################################################################
  ...
  while ( $cycleAnnotIter->hasNext() ) {
    ...
    if ( !defined $aggregateStats{'uniqCount'}->{$CN} ) {
      $aggregateStats{'uniqCount'}->{$CN} = 0;
      $aggregateStats{'totlength'}->{$CN} = 0 if !defined $aggregateStats{'totlength'}->{$CN};
    }
    ...
    # Class Name Stats
    $aggregateStats{'uniqCount'}->{$CN}++ unless $checked;
    $aggregateStats{'totlength'}->{$CN} += $length;
    $aggregateStats{'annotatedlength'} += $length;

    # Aggregate Stats
    my $cataloged = 0;
    foreach my $aggKey ( keys( %aggregateStats ) ) {
      if ( ref $aggregateStats{$aggKey} eq ref {} &&
           exists $aggregateStats{$aggKey}->{'RE'}) { 
  ...
  #
  # Now go through the dataset and renumber the ids starting
  # from 1
  #
  $cycleAnnotIter = $sortedAnnotationsList->getIterator();
  my %id = ();
  if ( ! exists $aggregateStats{'lastid'} ) {
    $aggregateStats{'lastid'} = 0;
  }
  while ( $cycleAnnotIter->hasNext() ) {
    my $currentAnnot = $cycleAnnotIter->next();
         ...
         && $currentAnnot->getLeftLinkedHit() != $currentAnnot )
    {
      ...
      $aggregateStats{'lastid'}++;
      print "Setting to new number " . ( $aggregateStats{'lastid'} ) . "\n" if ( $DEBUG );
      $id{$tmpID} = $aggregateStats{'lastid'};
    }
  }    # End while
       #printHitArrayList( $sortedAnnotationsList );
  ...
  if ( $options{'html'} ) {
    &printHTMLAnnots( "$file.out.html", \%id, \%seq1Lengths, \%colWidths,
                      $sortedAnnotationsList, $batchNum );
  }
  ...
  if ( $batchNum == 0 ) {
    ...
    open( OUTFULL, ">>$file.out" ) || die "can't append to $file.out\n";
  }
  if ( $options{'xm'} ) {
    if ( $batchNum == 0 ) {
      open( OUTXM, ">$file.out.xm" ) || die "can't create $file.out.xm\n";
    }else {
      open( OUTXM, ">>$file.out.xm" ) || die "can't append to $file.out.xm\n";
    }
  }
  if ( $options{'ace'} ) {
    if ( $batchNum == 0 ) {
      open( OUTACE, ">$file.out.ace" ) || die "can't create $file.out.ace\n";
    }else {
      open( OUTACE, ">>$file.out.ace" ) || die "can't append to $file.out.ace\n";
    }
  }
  if ( $options{'poly'} ) {
    if ( $batchNum == 0 ) {
      ...
      open( OUTPOLY, ">>$file.polyout" )
          || die "can't append to $file.out.polyout\n";
    }
  }
  if ( $options{'gff'} ) {
    if ( $batchNum == 0 ) {
      ...
      open( OUTGFF, ">>$file.out.gff" ) || die "can't append to $file.out.gff\n";
    }
  }
...
sub generateTableOutput {
  ...
  if ( -f $options{'maskSource'} ) {
    ...
    # Here $file.".out" should be the the full path to the temporary directory
    # the a file name of the form foo.fa.out
    ...
    $maskedlength  = $aggregateStats{'annotatedlength'};
    $maskedpercent = ( $maskedlength / $totseqlen ) * 100;
    $frac_GC       = "Unknown";
  }
  else {
  ...
  print "ProcessRepeats: Generating table output\n";
  my $totallength =
  ...
  if ( $options{'primate'} ) {
    ...
    printf $OUT "      ALUs       %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"SINE/Alu"}, $aggregateStats{'totlength'}->{"SINE/Alu"},
        $aggregateStats{'totlength'}->{"SINE/Alu"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      MIRs       %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"SINE/MIR"}, $aggregateStats{'totlength'}->{"SINE/MIR"},
        $aggregateStats{'totlength'}->{"SINE/MIR"} * 100 * $usePerc / $totseqlen;  
    printf $OUT "\n";
    ...
    printf $OUT "      LINE1      %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LINE/L1"}, $aggregateStats{'totlength'}->{"LINE/L1"},
        $aggregateStats{'totlength'}->{"LINE/L1"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      LINE2      %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LINE/L2"}, $aggregateStats{'totlength'}->{"LINE/L2"},
        $aggregateStats{'totlength'}->{"LINE/L2"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      L3/CR1     %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LINE/CR1"}, $aggregateStats{'totlength'}->{"LINE/CR1"},
        $aggregateStats{'totlength'}->{"LINE/CR1"} * 100 * $usePerc / $totseqlen;
    printf $OUT "\n";
    ...
    printf $OUT "      ERVL       %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LTR/ERVL"}, $aggregateStats{'totlength'}->{"LTR/ERVL"},
        $aggregateStats{'totlength'}->{"LTR/ERVL"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      ERVL-MaLRs %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LTR/ERVL-MaLR"}, $aggregateStats{'totlength'}->{"LTR/ERVL-MaLR"},
        $aggregateStats{'totlength'}->{"LTR/ERVL-MaLR"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      ERV_classI %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LTR/ERV1"}, $aggregateStats{'totlength'}->{"LTR/ERV1"},
        $aggregateStats{'totlength'}->{"LTR/ERV1"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      ERV_classII%6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"LTR/ERVK"}, $aggregateStats{'totlength'}->{"LTR/ERVK"},
        $aggregateStats{'totlength'}->{"LTR/ERVK"} * 100 * $usePerc / $totseqlen;
    printf $OUT "\n";
    ...
    printf $OUT "     hAT-Charlie %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"DNA/hAT-Charlie"}, $aggregateStats{'totlength'}->{"DNA/hAT-Charlie"},
        $aggregateStats{'totlength'}->{"DNA/hAT-Charlie"} * 100 * $usePerc / $totseqlen;
    printf $OUT "     TcMar-Tigger%6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"DNA/TcMar-Tigger"}, $aggregateStats{'totlength'}->{"DNA/TcMar-Tigger"},
        $aggregateStats{'totlength'}->{"DNA/TcMar-Tigger"} * 100 * $usePerc / $totseqlen;
    printf $OUT "\n";
    ...
    printf $OUT "      B2-B4       %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"SINE/B2"} + $aggregateStats{'uniqCount'}->{"SINE/B4"},
        $aggregateStats{'totlength'}->{"SINE/B2"} + $aggregateStats{'totlength'}->{"SINE/B4"},
        ( $aggregateStats{'totlength'}->{"SINE/B2"} + $aggregateStats{'totlength'}->{"SINE/B4"} ) * 100 * $usePerc /
        $totseqlen;
    printf $OUT "      IDs         %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"SINE/ID"}, $aggregateStats{'totlength'}->{"SINE/ID"},
        $aggregateStats{'totlength'}->{"SINE/ID"} * 100 * $usePerc / $totseqlen;
    printf $OUT "      MIRs        %6d   %10d bp   %5.2f \%\n",
        $aggregateStats{'uniqCount'}->{"SINE/MIR"}, $aggregateStats{'totlength'}->{"SINE/MIR"},
        $aggregateStats{'totlength'}->{"SINE/MIR"} * 100 * $usePerc / $totseqlen;
    printf $OUT "\n";
...
###############################################################################################################################
   ...
   my $OUT;
    ...
    printf $OUT "Class I Total:			%6d	%10d bp	%5.2f \%\n",
        ...
        $aggregateStats{'LTRLARDI'}->{'count'} +        
        $aggregateStats{'LTRTRIM'}->{'count'} +
        $aggregateStats{'LTRTRIMI'}->{'count'} +
	...
	#
        ...
        $aggregateStats{'LTRLARDI'}->{'length'} +                
        $aggregateStats{'LTRTRIM'}->{'length'} +
        $aggregateStats{'LTRTRIMI'}->{'length'} +
...
### Non-LTR 
    ...
    $aggregateStats{'SINE'}->{'count'} + $aggregateStats{'LINETOT'}->{'count'} ,
    $aggregateStats{'SINE'}->{'length'} + $aggregateStats{'LINETOT'}->{'length'} ,
    ($aggregateStats{'SINE'}->{'length'} + $aggregateStats{'LINETOT'}->{'length'})* 100 * $usePerc / $totseqlen;
    ...
    printf $OUT "|  |-LINEs:			%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINETOT'}->{'count'} + $aggregateStats{'LINEI'}->{'count'}, 
        $aggregateStats{'LINETOT'}->{'length'} + $aggregateStats{'LINEI'}->{'length'},
        ($aggregateStats{'LINETOT'}->{'length'} + $aggregateStats{'LINEI'}->{'length'}) * 100 * $usePerc / $totseqlen;


    printf $OUT "|    |-LINE-L1:			%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINEL1'}->{'count'},
        $aggregateStats{'LINEL1'}->{'length'},
        $aggregateStats{'LINEL1'}->{'length'} * 100 * $usePerc / $totseqlen;

    printf $OUT "|    |-LINE-L2:			%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINEL2'}->{'count'},
        $aggregateStats{'LINEL2'}->{'length'},
        $aggregateStats{'LINEL2'}->{'length'} * 100 * $usePerc / $totseqlen;


    printf $OUT "|    |-LINE-Penelope:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELPEL'}->{'count'},
        $aggregateStats{'LINELPEL'}->{'length'},
        $aggregateStats{'LINELPEL'}->{'length'} * 100 * $usePerc / $totseqlen;


    printf $OUT "|    |-LINE-CR1:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELCR1'}->{'count'},
        $aggregateStats{'LINELCR1'}->{'length'},
        $aggregateStats{'LINELCR1'}->{'length'} * 100 * $usePerc / $totseqlen;


    printf $OUT "|    |-LINE-R1:			%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELR1'}->{'count'},
        $aggregateStats{'LINELR1'}->{'length'},
        $aggregateStats{'LINELR1'}->{'length'} * 100 * $usePerc / $totseqlen;


    printf $OUT "|    |-LINE-RTE:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELRTE'}->{'count'},
        $aggregateStats{'LINELRTE'}->{'length'},
        $aggregateStats{'LINELRTE'}->{'length'} * 100 * $usePerc / $totseqlen;
        
        
    printf $OUT "|    |-LINE-Tad1:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELTAD'}->{'count'},
        $aggregateStats{'LINELTAD'}->{'length'},
        $aggregateStats{'LINELTAD'}->{'length'} * 100 * $usePerc / $totseqlen;        


    printf $OUT "|    |-LINE-Daphne:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELDAP'}->{'count'},
        $aggregateStats{'LINELDAP'}->{'length'},
        $aggregateStats{'LINELDAP'}->{'length'} * 100 * $usePerc / $totseqlen;    


    printf $OUT "|    |-LINE-Hero:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELHERO'}->{'count'},
        $aggregateStats{'LINELHERO'}->{'length'},
        $aggregateStats{'LINELHERO'}->{'length'} * 100 * $usePerc / $totseqlen; 


    printf $OUT "|    |-LINE-Jockey:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELJOCK'}->{'count'},
        $aggregateStats{'LINELJOCK'}->{'length'},
        $aggregateStats{'LINELJOCK'}->{'length'} * 100 * $usePerc / $totseqlen; 


    printf $OUT "|    |-LINE-NeSL:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELNESL'}->{'count'},
        $aggregateStats{'LINELNESL'}->{'length'},
        $aggregateStats{'LINELNESL'}->{'length'} * 100 * $usePerc / $totseqlen; 
        
        
    printf $OUT "|    |-LINE-Nimb:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELNIMB'}->{'count'},
        $aggregateStats{'LINELNIMB'}->{'length'},
        $aggregateStats{'LINELNIMB'}->{'length'} * 100 * $usePerc / $totseqlen;         
        

    printf $OUT "|    |-LINE-RandI:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELRAND'}->{'count'},
        $aggregateStats{'LINELRAND'}->{'length'},
        $aggregateStats{'LINELRAND'}->{'length'} * 100 * $usePerc / $totseqlen; 
        
        
    printf $OUT "|    |-LINE-Tx1:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELTX1'}->{'count'},
        $aggregateStats{'LINELTX1'}->{'length'},
        $aggregateStats{'LINELTX1'}->{'length'} * 100 * $usePerc / $totseqlen;         
        

    printf $OUT "|    |-LINE-RTETP:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELRTETP'}->{'count'},
        $aggregateStats{'LINELRTETP'}->{'length'},
        $aggregateStats{'LINELRTETP'}->{'length'} * 100 * $usePerc / $totseqlen;    

    printf $OUT "|    |-LINE-CRE:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LINELCRE'}->{'count'},
        $aggregateStats{'LINELCRE'}->{'length'},
        $aggregateStats{'LINELCRE'}->{'length'} * 100 * $usePerc / $totseqlen;  
...
### LTR - Non Autonomous 
    ...
    $aggregateStats{'LTRLARD'}->{'count'} + $aggregateStats{'LTRTRIM'}->{'count'} + $aggregateStats{'LTRTRGAG'}->{'count'} + $aggregateStats{'LTRBARE2'}->{'count'} + $aggregateStats{'LTRLARDI'}->{'count'} + $aggregateStats{'LTRTRIMI'}->{'count'},
    $aggregateStats{'LTRLARD'}->{'length'} + $aggregateStats{'LTRTRIM'}->{'length'} + $aggregateStats{'LTRTRGAG'}->{'length'} + $aggregateStats{'LTRBARE2'}->{'length'} + $aggregateStats{'LTRLARDI'}->{'length'} + $aggregateStats{'LTRTRIMI'}->{'length'},
    ($aggregateStats{'LTRLARD'}->{'length'} + $aggregateStats{'LTRTRIM'}->{'length'} + $aggregateStats{'LTRTRGAG'}->{'length'} + $aggregateStats{'LTRBARE2'}->{'length'} + $aggregateStats{'LTRLARDI'}->{'length'} + $aggregateStats{'LTRTRIMI'}->{'length'})* 100 * $usePerc / $totseqlen;
    ...
    printf $OUT "|   |  | |-LARDs-like:		%6d	%10d bp	%5.2f \%\n",
#    printf $OUT "           |-LARDs-like:		%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LTRLARDI'}->{'count'},
        $aggregateStats{'LTRLARDI'}->{'length'},
        $aggregateStats{'LTRLARDI'}->{'length'} * 100 * $usePerc / $totseqlen;
    ...
    printf $OUT "|   |  | |-TRIMs-like:		%6d	%10d bp	%5.2f \%\n",
#    printf $OUT "           |-TRIMs:			%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LTRTRIMI'}->{'count'},
        $aggregateStats{'LTRTRIMI'}->{'length'},
        $aggregateStats{'LTRTRIMI'}->{'length'} * 100 * $usePerc / $totseqlen;
...
### LTR - Copia
    ...
    printf $OUT "|     |  | |-Alexandra-like:	%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LTRALEXI'}->{'count'},
        $aggregateStats{'LTRALEXI'}->{'length'},
        $aggregateStats{'LTRALEXI'}->{'length'} * 100 * $usePerc / $totseqlen;
    ...
    printf $OUT "|     |  | |-Ferco-like:	%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LTRFERCOI'}->{'count'},
        $aggregateStats{'LTRFERCOI'}->{'length'},
        $aggregateStats{'LTRFERCOI'}->{'length'} * 100 * $usePerc / $totseqlen;
    ...
    printf $OUT "|     |  | |-Bryana-like:	%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LTRBRYANI'}->{'count'},
        $aggregateStats{'LTRBRYANI'}->{'length'},
        $aggregateStats{'LTRBRYANI'}->{'length'} * 100 * $usePerc / $totseqlen;
...
### LTR - Gypsy
   ...
   #printf $OUT "------------------------LTR Gypsy--------------------------\n";
    ...
    printf $OUT "|       |  |  |-outgroup-like:	%6d	%10d bp	%5.2f \%\n",
    ...
    printf $OUT "|       |     | |-Athila-like:	%6d	%10d bp	%5.2f \%\n",
    ...
    printf $OUT "|       |     | |-Tatius-like:	%6d	%10d bp	%5.2f \%\n",
    ...
    printf $OUT "|       |        |-TatI:	%6d	%10d bp	%5.2f \%\n",
        ...
        $aggregateStats{'LTRTATI'}->{'length'},
        $aggregateStats{'LTRTATI'}->{'length'} * 100 * $usePerc / $totseqlen;
    printf $OUT "|       |        |  |-like:	%6d	%10d bp	%5.2f \%\n",
    ...
    printf $OUT "|          |  |-Ferney-like:	%6d	%10d bp	%5.2f \%\n",
        $aggregateStats{'LTRFERNI'}->{'count'},
        $aggregateStats{'LTRFERNI'}->{'length'},
        $aggregateStats{'LTRFERNI'}->{'length'} * 100 * $usePerc / $totseqlen;
