...
```sh
sudo docker pull annotep-local-interface:v1
```

**Step 2.** Next, run the container with the command below, replacing ``/path/directory/results`` with the directory where you want to store the results generated by the annotation:
```sh
sudo docker run -it -v /path/directory/results:/root/TEs/www/results --name local-interface -dp 0.0.0.0:5000:5000 annotep-local-interface:v1
```
...
Step 5. Within the interface you can enter your data such as: email, genome and annotation type and send it for analysis. When the work is completed without any errors, you will receive an e-mail informing you that the results are available in the directory in ``/path/directory/results``.
...
**Step 1.** Download the AnnoTEP image:
```sh
sudo docker pull annotep-local-terminal:v1
```

**Step 2.** Use the ``-h`` parameter to display a user guide describing how to use the script:

```sh
sudo docker run annotep-local-terminal:v1 python run_annotep.py -h
```
...
**Step 3.** Next, run the container with the command below, replacing ``/path/directory/results`` with the directory where you want to store the results generated by the annotation, and ``/path/directory/genome`` with the directory where your genome is located:

```sh
sudo docker run -it -v /path/directory/results:/root/TEs/local/results -v /home/user/TEs:/path/directory/genome annotep-local-terminal:v1 python run_annotep.py --file {/path/directory/genome/genome.fasta} --type {type-annotation}
```

Example:
```sh
sudo docker run -it -v $HOME/results:/root/TEs/local/results -v $HOME/TEs:$HOME/TEs annotep-local-terminal:v1 python run_annotep.py --file $HOME/TEs/genome.fasta --type 2
```
