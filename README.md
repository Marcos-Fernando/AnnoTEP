...
Step 2. Next, run the container with the command below, replacing ``/path/directory/results`` with the directory where you want to store the results generated by the annotation, and ``/path/directory/genome`` with the directory where your genome is located:

```sh
    sudo docker run -it -v /path/directory/results:/root/TEs/local/results -v /home/user/TEs:/path/directory/genome "nome_da_imagem" python run_annotep.py --file {/path/directory/genome/genome.fasta} --type {type-annotation}
```

- In the ``-file`` parameter, add the same directory as the genome followed by the name of the genome to be used in .fasta format.
- In the ``--type`` parameter, enter the number corresponding to the type of annotation you want to perform:
[1] SINE Annotation 
[2] LINE Annotation
